"""Parse the OpenTelemetry span export into one record per agent run.

Every extractor takes a list of candidate attribute names and uses the first one
present, which is what keeps a GenAI semantic-convention rename from silently
emptying the table.
"""
...
def _tool_names(attributes: dict) -> list[str]:
    """Names of the tools offered to the model on this call, if it recorded them."""
...
def _airflow_context(span: dict, by_span_id: dict) -> dict:
    """Walk parents until a span with Airflow task attributes is found.

    Core tracing puts dag_id, task_id, run_id, map_index and try_number on the task
    spans that GenAI spans nest under, never on the GenAI spans themselves.
    """
    seen: set = set()
...
def _ancestor(span: dict, by_span_id: dict, roots: set) -> str | None:
    """Walk parents until an agent-run span is reached.

    Airflow opens one trace per Dag run, so tool calls cannot be grouped by trace id:
    they have to be attributed to the agent run they descend from.
    """
    seen = set()
...
def read_agent_runs(path: Path = SPANS_FILE) -> list[dict]:
    """One record per agent run, with its final output and its tool calls.

    A single run makes several model calls, and the completion on an intermediate one
    is a tool call, so a model-call span is the wrong unit to evaluate.
    """
